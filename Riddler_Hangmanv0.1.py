    ...
    while len(words) > 0:
        ...
        if guess not in words:
            ...
            if turns == 1:
                ...
                print("   / \   ")
            if turns == 0:
                print("How many Bats does it take to solve my challenge? Obviously more than one. You've let an innocent man die Dark Knight.")
                print(" ------- ")
                print("      |  ")
                print("    O-   ")
                print("   /|\   ")
...
def riddle():
    ...
    turns = 3
    riddles = {
        "\nHit me hard and I will crack, but you'll never stop me from staring back. What am i?" : 'mirror', 
    ...
    x = random.choice(list(riddles))
    
    print(x)
    
    guess = ""
    while guess != riddles.get(x):
        guess = input().lower()
        turns -= 1
        if guess != riddles.get(x):
            if turns == 2:
